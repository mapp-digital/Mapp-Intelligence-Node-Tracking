...
import {ConsumerForkCurl} from '../../../src/consumer/ConsumerForkCurl';

describe('ConsumerForkCurl', () => {
    ...
    it('batch request responding 200 via HTTPS', async () => {
        ...
        const consumer = new ConsumerForkCurl(mic.build());

        const data = ['wt?p=300,0'];
        expect(await consumer.sendBatch(data)).toBeTruthy();

        const message: string = customLogger.getMessages();
        expect(message).toContain("Send batch data to https://analytics01.wt-eu02.net/123451234512345/batch (1 req.)");
    ...
    it('batch request responding timeout exception', async () => {
        ...
        const message: string = customLogger.getMessages();
        expect(message.search(/error\s\(\d+\)/ig)).toBeGreaterThan(-1);
    });
    ...
    it('batch request responding 200 via HTTP', async () => {
        ...
        const message: string = customLogger.getMessages();
        expect(message).toContain("Send batch data to http://analytics01.wt-eu02.net/123451234512345/batch (1 req.)");
        expect(message).toContain("Batch request responding the status code 200");
    });
    ...
    it('batch request responding 404', async () => {
        ...
        const message: string = customLogger.getMessages();
        expect(message).toContain("Send batch data to https://analytics01.wt-eu02.net/111111111111111/batch (1 req.)");
